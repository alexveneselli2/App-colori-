  ...
  if (sorted.length === 0) {
    return (
      <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:200, gap:8 }}>
        ...
        <p style={{ fontSize:15, fontWeight:700, color:'var(--color-foreground)' }}>{t.timeline_empty}</p>
        <p style={{ fontSize:13, color:'var(--color-muted)', textAlign:'center' }}>{t.timeline_empty_sub}</p>
      </div>
    )
  }
  ...
  return (
    <div style={{ position:'relative', width:'100%' }}>
      ...
      <p style={{ textAlign:'center', marginTop:10, fontSize:11, color:'var(--color-muted)' }}>
        {sorted.length} {sorted.length === 1 ? t.timeline_day : t.timeline_days}
      </p>
    </div>
  )
}
